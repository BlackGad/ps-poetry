...
Register custom implementations with the [DI container](https://github.com/BlackGad/ps-poetry/blob/main/libraries/di/README.md) using `di.register(ICheck)` inside `poetry_activate` so the check module discovers them automatically via `di.resolve_many(ICheck)`.
