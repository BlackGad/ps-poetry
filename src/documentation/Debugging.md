...
The workspace includes three pre-configured launch profiles:

* **Launcher debugpy** — injects and debugs the plugin in a target project (the primary debugging profile)
* **Direct debugpy** — runs `poetry sync` directly in the workspace without plugin injection
* **Experiment** — runs the currently open file with the debugger attached (for quick experiments)

## How to Use

1. Open the Run and Debug panel in VS Code (Ctrl+Shift+D)
2. Select **"Launcher debugpy"** from the dropdown
3. Click the Start Debugging button (F5)
4. When prompted, provide:
   * **Target project path**: Absolute or relative path to the project where the plugin should be tested (defaults to `.`)
   * **Target command line arguments**: The Poetry command line to execute (e.g., `install`, `update`, `env info`)
